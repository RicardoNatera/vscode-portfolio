...
export default function TabBar({ openTabs, activeFile, onSelect, onClose }: Props) {
  return (
    <div className="flex h-10 bg-[#2d2d2d] border-b border-[#444]">
      {openTabs.map((file) => {
        const isActive = activeFile === file;
        return (
          <div
            key={file}
            className={`relative group flex items-center gap-2 px-4 cursor-pointer text-sm border-r border-[#444] ${
          ...
          >
            ...
            <span
              className={`
                ml-2 transition-opacity rounded-sm p-[2px]
